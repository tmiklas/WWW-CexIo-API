...
=head2 cencel_order($order_id)

Cancels order with given ID number.

	# cancel order 1234567 - use C<open_orers()> to find order numbers
...
Defines where we post our trade offer - this would currently be one of GHS/BTC, NMC/BTC, GHS/NMC or BF1/BTC.
...
=head2 instant_order(%hash)

Posts market matching order to ensure the transaction goes through immediately. Example:

	# change all NMC for BTC at current market rate
	my $balance = $api->balance;
	my $result = $api->instant_order(
		'pair' => 'NMC/BTC', 
		'type' => 'sell', 
		'amount' => $balance->{NMC}->{available},
	);
 
This is just a convenience feature that will do everything for you in the background, so you 
don't have to calculate manually how much of commodity you get.
